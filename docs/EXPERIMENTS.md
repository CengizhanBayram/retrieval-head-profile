...
| Exp | What | Code | NB | Out |
|---|---|---|---|---|
| **E6** | NIAH behavioural sweep: contexts × positions, "lost-in-the-middle" depth | `src.NIAHEvaluator.evaluate` via `scripts._common.run_behavior_for_model` | 00, 02 | `behavior/<m>.json` → `niah_matrix`, `niah_overall`, `niah_worst_pos` |
| **E7** | RULER subset: multi-key, multi-value, variable tracking | `rhp.ruler.RulerEvaluator.run` | 00, 02 | `behavior.ruler` |
| **E8** | Profile → behaviour prediction: BH-corrected Spearman, LOO ≤3-predictor regression, family-demeaned | `rhp.prediction.run_prediction_analysis` | 04 (CPU) | `analysis/prediction_e8.json` |
